...
// Fixed icon mapping for the three Portugal climate zones in display order
...
export function ClimateSection({ title, points }: ClimateSectionProps): React.JSX.Element {
  return (
    <section
      className="py-16 lg:py-24 bg-navy text-white"
      ...
      <Container>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {points.map((point, index) => {
            // Cycle through icons if more than 3 points are provided
            const ZoneIcon = ZONE_ICONS[index % ZONE_ICONS.length]

            return (
              <div
                key={index}
                className="flex flex-col items-center text-center bg-white/8 border border-white/15 rounded-xl px-6 py-8"
              >
                {/* Icon */}
                ...
                <p className="text-white/70 leading-relaxed text-sm">{point.description}</p>
              </div>
            )
          })}
        </div>
